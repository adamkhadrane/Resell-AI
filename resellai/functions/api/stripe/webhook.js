...
// Handles Stripe webhook events and syncs to Supabase
...
export async function onRequestPost(context) {
  ...
  async function supabaseGetByCustomer(customerId) {
  ...
  try {
    switch (event.type) {
      ...
      case "customer.subscription.updated": {
        ...
        const userId = await supabaseGetByCustomer(sub.customer);
        if (!userId) break;
      ...
      case "invoice.payment_failed": {
        ...
        const userId = await supabaseGetByCustomer(invoice.customer);
        if (!userId) break;
...
async function verifyStripeWebhook(payload, sigHeader, secret) {
  ...
  if (computedSig !== signature) throw new Error("Signature mismatch");
