// api/ebay/search.js — Vercel Edge Function
...
export default async function handler(request) {
  ...
  try {
    ...
    if (!accessToken) {
      return new Response(JSON.stringify({ error: 'eBay auth failed' }), {
        status: 500, headers: corsHeaders,
      });
    }

    const categoryMap = {
      sneakers: '15709', clothing: '11450', electronics: '58058',
      cards: '2536', collectibles: '1',
    };
    const catId = categoryMap[category] || '';
    const catParam = catId ? `&category_ids=${catId}` : '';

    const searchRes = await fetch(
      `https://api.ebay.com/buy/browse/v1/item_summary/search?q=${encodeURIComponent(query)}${catParam}&limit=${limit}&sort=PRICE_DESC`,
      {
        headers: {
    ...
    const flips = items
      ...
      .map((item) => {
        ...
        const platformFee = currentPrice * 0.1295;
        const shipping = 8.99;
